...
int getSerial1Time()
{
  ...
  char c;
  int r_index = 0;
  int ser_hours = 0, ser_minutes = 0, ser_seconds= 0, ser_day=0, ser_month=0, ser_year=0;
  char temp1[3], temp2[3], temp3[3], temp4[3], temp5[3], temp6[5];

  if(Serial.available() > 0)
  {

    r_index = Serial.readBytesUntil('\n',r_data,100);
    r_data[r_index] = '\0';

    //Serial.println(r_data);

    for(int i = 0; i < strlen(r_data); i++)
    {
      switch(r_data[i])
      {
        case 'H':
          count++;
          strncpy(temp1, &r_data[i+1], 2);
          ser_hours = atoi(temp1);
        break;

        case 'N':
            count++;
           strncpy(temp2, &r_data[i+1], 2);
           ser_minutes = atoi(temp2);
        break;

        case 'S':
            count++;
           strncpy(temp3, &r_data[i+1], 2);
           ser_seconds = atoi(temp3);
        break;

        case 'D':
          count++;
           strncpy(temp4, &r_data[i+1], 2);
           ser_day = atoi(temp4);
        break;

        case 'M':
          count++;
           strncpy(temp5, &r_data[i+1], 2);
           ser_month = atoi(temp5);
        break;

        case 'Y':
          count++;
              strncpy(temp6, &r_data[i+1], 4);
              ser_year = atoi(temp6);
        break;
      }
    }
  ...
  /*  Serial.print("Read time ");
    Serial.print(ser_hours);
    Serial.print(":");
    Serial.print(ser_minutes);
    Serial.print(":" );
    Serial.print(ser_seconds);


    Serial.print(" ");
    Serial.print(ser_day);
    Serial.print("-");
    Serial.print(ser_month);
    Serial.print("-" );
    Serial.println(ser_year);*/

     setRTCDateTime(ser_hours, ser_minutes, ser_seconds, ser_day, ser_month, ser_year % 1000, 1);
     getRTCTime();
     setTime(RTC_hours, RTC_minutes, RTC_seconds, RTC_day, RTC_month, RTC_year);
    }
  }
...
void doSync(){
  char r_data[5];
  int sync_command = -1;
  int r_index, result = 0;
  unsigned long timeout = 0, start;
   ...
   if(Serial.available() > 0)
    {
     r_index = Serial.readBytesUntil('\n',r_data,10);
     r_data[r_index] = '\0';
     sync_command = strcmp(r_data,"SYNC");
    }

  if(sync_command == 0)
  {
    Serial.print("WAIT FOR SYNC\n");
    start = millis();
    while(!result && timeout < 10000) 
    {
      result = getSerial1Time();
      timeout = millis() - start;  
    }
    if(result)Serial.print("SYNC OK\n");
    else Serial.print("TIMEOUT OR SYNC FAIL\n");
    sync_command = -1;
  }
}
